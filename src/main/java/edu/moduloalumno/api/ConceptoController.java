...
public class ConceptoController {
	...
	public ResponseEntity<List<Ubicacions>> getAllUbicacion(){
		logger.info("> getConceptoApellidosNombres [Concepto]");

		List<Ubicacions> list = null;

		try {
			list = service.getAllUbicacion();
			
			if (list == null) {
				list = new ArrayList<Ubicacions>();
			}
		} catch (Exception e) {
			logger.error("Unexpected Exception caught.", e);
			return new ResponseEntity<List<Ubicacions>>(list, HttpStatus.INTERNAL_SERVER_ERROR);
		}
		

		logger.info("< getConceptoApellidosNombres [Concepto]");
		return new ResponseEntity<List<Ubicacions>>(list, HttpStatus.OK);
	}
	...
	public ResponseEntity<List<Cuentas>> getAllCuentas(){
		...
		List<Cuentas> list = null;

		try {
			list = service.getAllCuentas();
			
			if (list == null) {
				list = new ArrayList<Cuentas>();
			}
		} catch (Exception e) {
			logger.error("Unexpected Exception caught.", e);
			return new ResponseEntity<List<Cuentas>>(list, HttpStatus.INTERNAL_SERVER_ERROR);
		}
		

		logger.info("< getConceptoApellidosNombres [Concepto]");
		return new ResponseEntity<List<Cuentas>>(list, HttpStatus.OK);
	}*/
}
